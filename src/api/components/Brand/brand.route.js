...
const { createBrand, updateBrand, getSingleBrand, getAllBrands, deleteBrand } = require('./brand.controller');

const { authenticateUser, isAdmin } = require('../../middlewares/authenticate');

router.route('/').post(authenticateUser, isAdmin, createBrand).get(getAllBrands);
